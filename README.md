...
`a2.tablename.mapper` - The fully-qualified class name of the class that computes name of Cassandra table based on information from Kafka topic. The default [solutions.a2.kafka.cassandra.DefaultKafkaToCassandraTableNameMapper](https://github.com/averemee-si/cassandra-kafka-connect/blob/main/src/main/java/solutions/a2/kafka/cassandra/DefaultKafkaToCassandraTableNameMapper.java) uses topic name as Cassandra table name when `a2.schema.type` set to **plain** or **key_value** and value of source.table field when a2.schema.type set to **debezium**. To override this default you need to create a class that implements the [solutions.a2.kafka.cassandra.KafkaToCassandraTableNameMapper](https://github.com/averemee-si/cassandra-kafka-connect/blob/main/src/main/java/solutions/a2/kafka/cassandra/KafkaToCassandraTableNameMapper.java) interface

`a2.kafka.key.fields.getter` - The fully-qualified class name of the class that extracts list of key fields definitions from Kafka SinkRecord. The default [solutions.a2.kafka.cassandra.DefaultKafkaKeyFieldsGetter](https://github.com/averemee-si/cassandra-kafka-connect/blob/main/src/main/java/solutions/a2/kafka/cassandra/DefaultKafkaKeyFieldsGetter.java) just returns fields of keySchema() when `a2.schema.type` set to **key_value** or **debezium**, and list of all non-optional fields when `a2.schema.type` set to **plain**. To override this default you need to create a class that implements the [solutions.a2.kafka.cassandra.KafkaKeyFieldsGetter](https://github.com/averemee-si/cassandra-kafka-connect/blob/main/src/main/java/solutions/a2/kafka/cassandra/KafkaKeyFieldsGetter.java) interface

`a2.kafka.value.fields.getter` - The fully-qualified class name of the class that extracts list of value fields definitions from Kafka SinkRecord. The default [solutions.a2.kafka.cassandra.DefaultKafkaValueFieldsGetter](https://github.com/averemee-si/cassandra-kafka-connect/blob/main/src/main/java/solutions/a2/kafka/cassandra/DefaultKafkaValueFieldsGetter.java) just returns fields of valueSchema() when `a2.schema.type` set to **key_value** or **debezium**, and list of all optional fields when `a2.schema.type` set to **plain**. To override this default you need to create a class that implements the [solutions.a2.kafka.cassandra.KafkaValueFieldsGetter](https://github.com/averemee-si/cassandra-kafka-connect/blob/main/src/main/java/solutions/a2/kafka/cassandra/KafkaValueFieldsGetter.java) interface

`a2.kafka.delete.event.qualifier` - The fully-qualified class name of the class that determines that source operation was deletion. The default [solutions.a2.kafka.cassandra.DefaultKafkaDeleteEventQualifier](https://github.com/averemee-si/cassandra-kafka-connect/blob/main/src/main/java/solutions/a2/kafka/cassandra/DefaultKafkaDeleteEventQualifier.java) returns true if SinkRecord header **'op'** set to **'d'** or **'D'**, or when `a2.schema.type` set to **debezium** when **'op'** field set to **'d'** or always false when `a2.schema.type` set to **plain**. To override this default you need to create a class that implements the [solutions.a2.kafka.cassandra.KafkaDeleteEventQualifier](https://github.com/averemee-si/cassandra-kafka-connect/blob/main/src/main/java/solutions/a2/kafka/cassandra/KafkaDeleteEventQualifier.java) interface
...
`a2.offset.flush.batch` - Specifies the number of Sink records before the call of SinkTask.flush(). Default - **100**
